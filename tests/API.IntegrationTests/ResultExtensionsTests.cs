...
public class ResultExtensionsTests
{
    ...
    public async Task ToProblemDetails_NotFound_Returns404()
    {
        ...
        var status = await GetStatusCodeAsync(result.ToProblemDetails());
    ...
    public async Task ToProblemDetails_Validation_Returns400()
    {
        ...
        var status = await GetStatusCodeAsync(validation.ToProblemDetails());
    ...
    {
        var result = Result.Success();
        var status = await GetStatusCodeAsync(result.ToHttpResult());
    ...
    public async Task ToCreatedAt_Success_Returns201()
    {
        ...
        var status = await GetStatusCodeAsync(result.ToCreatedAt("/api/v1/items/1"));
        status.Should().Be((int)HttpStatusCode.Created);
    }

    [Fact]
    public async Task ToHttpResult_Failure_DelegatesToProblemDetails()
    {
        var result = Result.Failure(ErrorCodes.Tutor.NotFound);
        var status = await GetStatusCodeAsync(result.ToHttpResult());
        status.Should().Be((int)HttpStatusCode.NotFound);
    }
}
